...
def get_generations(model_name:str, args, seed=1, old_sequences=None, max_num_gen_once=args.num_generations_per_prompt):
    ...
    for _, batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        while num_gens > 0:
            ...
            scores_pred = dict_outputs.scores # list which have seq_len elements which size is (batch_size, vocab_size)
            num_generated_tokens = len(scores_pred)
            generation = dict_outputs.sequences[:, input_length:input_length+num_generated_tokens].cpu() # slice so that only token ids of answer part are left (batch_size, seq_len)
            generations.append(generation) 
            num_tokens = get_num_tokens(generation) # the number of generated tokens per generation
            predictive_entropy = get_lenghthNormalized_entropy(scores_pred, num_tokens) # baseline: LN entropy
